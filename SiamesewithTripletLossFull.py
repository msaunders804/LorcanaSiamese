...
tripletsTrain_tensor = tf.convert_to_tensor(tripletsTrain_flat, dtype=tf.float32)
...
tripletsTest_tensor = tf.convert_to_tensor(tripletsTest_flat, dtype=tf.float32)

# Create and compile the Siamese network
input_shape = (224, 312, 3)  # Adjust based on your image size
siamese_net = create_siamese_network(input_shape)
siamese_net.compile(optimizer=optimizers.Adam(), loss=TripletLoss())

# Train the Siamese network
history = siamese_net.fit([tripletsTrain_tensor[:, 0], tripletsTrain_tensor[:, 1], tripletsTrain_tensor[:, 2]],
                          np.zeros(len(tripletsTrain_tensor)),  # Dummy labels, as triplet loss doesn't need labels
                          validation_data=([tripletsTest_tensor[:, 0], tripletsTest_tensor[:, 1], tripletsTest_tensor[:, 2]],
                                           np.zeros(len(tripletsTest_tensor))),
                          batch_size=32, epochs=10)  # Adjust batch_size and epochs as needed
